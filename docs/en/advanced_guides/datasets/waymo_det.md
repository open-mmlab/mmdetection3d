...
`pklfile_prefix` should be set in `WaymoMetric` in `test_evaluator` of configuration if the bin file is needed to be generated.
...
`submission_prefix` should be set in `WaymoMetric` in `test_evaluator` of configuration before you run the test command  if you want to generate the bin files and make a submission to the leaderboard..

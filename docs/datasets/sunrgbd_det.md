...
- `sunrgbd_infos_train.pkl`: The train data infos, the detailed info of each scene is as follows:
    ...
    - info['annos']: The annotations of each scan.
        ...
        - annotations['gt_boxes_upright_depth']: The 3D bounding boxes in depth coordinate system, each bounding box is (x, y, z, x_size, y_size, z_size), shape: [K, 7].
...
- Data augmentation for point clouds:
    ...
    - `GlobalRotScaleTrans`: rotate the input point cloud, usually in the range of [-30, 30] (degrees) for SUN RGB-D.

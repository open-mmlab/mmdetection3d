...
def determine_gpus(cfg_name):
    ...
    if cfg_name.find('x16') >= 0:
        gpus = 16
    elif cfg_name.find('x4') >= 0:
        gpus = 4
        gpus_per_node = 4
    elif 'pointnet2' in cfg_name:
        gpus = 2
        gpus_per_node = 2

    return gpus, gpus_per_node
...
def main():
    ...
    with open(args.txt_path, 'r') as f:
        ...
        for i, cfg in enumerate(model_cfgs):
            ...
            gpus, gpus_per_node = determine_gpus(cfg)
            command_info = f'GPUS={gpus}  GPUS_PER_NODE={gpus_per_node}  ' \

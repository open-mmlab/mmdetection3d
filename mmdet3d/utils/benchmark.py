...
class InferenceBenchmark(BaseBenchmark):
            ...
            if i >= self.num_warmup:
                ...
                if (i + 1) % self.log_interval == 0:
                    ...
                    print_log(
                        ...
                        f'times per image: {1000 / fps:.1f} ms/sample, '
    ...
    def average_multiple_runs(self, results: List[dict]) -> dict:
        ...
        if len(fps_list_) > 1:
            ...
            print_log(
                f'Overall fps: {fps_list_}[{avg_fps_:.1f}] sample/s, '
                'times per image: '
                f'{times_pre_image_list_}[{avg_times_pre_image_:.1f}] '
                'ms/sample', self.logger)
        else:
            print_log(
                f'Overall fps: {fps_list_[0]:.1f} sample/s, '
                f'times per image: {1000 / fps_list_[0]:.1f} ms/sample',
                self.logger)
...
class DatasetBenchmark(BaseBenchmark):
        ...
        for i, idx in enumerate(total_index):
            ...
            if i >= self.num_warmup:
                ...
                if (i + 1) % self.log_interval == 0:
                    ...
                    print_log(
                        f'Done sample [{i + 1:<3}/{self.max_iter}], '
                        f'fps: {fps:.1f} sample/s, '
                        f'times per sample: {1000 / fps:.1f} ms/sample',
                        self.logger)
    ...
    def average_multiple_runs(self, results: List[dict]) -> dict:
        ...
        if len(fps_list_) > 1:
            ...
            print_log(
                f'Overall fps: {fps_list_[0]:.1f} sample/s, '
                f'times per sample: {1000 / fps_list_[0]:.1f} ms/sample',
                self.logger)

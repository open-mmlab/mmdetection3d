...
def get_kitti_2d_boxes(info, occluded, mono3d=True):
    ...
    camera_intrinsic = info['calib']['P2']
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        center_loc = loc + dim * (dst - src)
        # TODO: keep offset for performance matching
        offset = (info['calib']['P2'][0, 3] - info['calib']['P0'][0, 3]) \
            / info['calib']['P2'][0, 0]
        loc_3d = np.copy(loc)
        loc_3d[0, 0] += offset
        gt_bbox_3d = np.concatenate([center_loc, dim, rot],
        ...
        # Generate dictionary record to be included in the .json file.
        repro_rec = generate_kitti_mono3d_record(ann_rec, min_x, min_y, max_x,
        ...
        if mono3d and (repro_rec is not None):
            ...
            center_3d = np.array(center_loc).reshape([1, 3])
...
def generate_kitti_mono3d_record(ann_rec, x1, y1, x2, y2, sample_data_token,
                                 filename):
    """Generate one 2D annotation record given various information on top of
    the 2D bounding box coordinates.

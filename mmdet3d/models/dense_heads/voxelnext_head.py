...
class VoxelNeXt_SeparateHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(VoxelNeXt_SeparateHead, self).__init__(init_cfg=init_cfg)
        self.heads = heads
        ...
        for head in self.heads:
            ...
            for i in range(num_conv - 1):
                conv_layers.append(SparseSequential(
                    SubMConv2d(c_in, c_in, 1, padding=0, bias=True, indice_key=head),
                    nn.BatchNorm1d(c_in),
                    nn.ReLU()
                ))
            conv_layers.append(SubMConv2d(c_in, classes, 1, bias=True, indice_key=head+'out'))
    ...
    def forward(self, x):
        ...
        ret_dict = dict()
        for head in self.heads:
            ret_dict[head] = self.__getattr__(head)(x).features

        return ret_dict

@MODELS.register_module()
class VoxelNeXtHead(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(VoxelNeXtHead, self).__init__(init_cfg=init_cfg, **kwargs)
    ...
    def forward_single(self, x: Tensor) -> dict:
        ...
        """
        ret_dicts = []
        # print('4d shape?', x.shape) #torch.Size([128, 16, 16])
        # x = x.unsqueeze(0)
        # print('4d shape?', x.shape)
        # x = self.shared_conv(x) #暂时先码了
    ...
    def predict(self,
        ...
        Returns:
            ...
            scores and labels.
        """
        spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels = self._get_voxel_infos(pts_feats)
        ...
        results_list = self.predict_by_feat(
            preds_dict, batch_input_metas, voxel_indices, spatial_shape, 
    ...
    def predict_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            if self.double_flip:
                ...
                self.merge_double_flip(preds_dict, batch_size, voxel_indices.clone(), spatial_shape)
            else:
                num_class_with_bg = self.num_classes[task_id]
                batch_size = voxel_indices[-1, 0].item() + 1
                batch_hm = preds_dict[0]['heatmap'].sigmoid()
                ...
                batch_rot_cos = preds_dict[0]['rot'][:, 0].unsqueeze(1)
                batch_rot_sin = preds_dict[0]['rot'][:, 1].unsqueeze(1)

                if self.iou_branch:
                    batch_iou = (preds_dict['iou'] + 1) * 0.5
                else:
                    batch_iou = None
                ...
                temp = self.bbox_coder.decode(
                    ...
                    obj=batch_hm, 
                    rot_sine=batch_rot_sin,
                    rot_cosine=batch_rot_cos,
                    hei=batch_hei,
                    dim=batch_dim,
                    vel=batch_vel, 
                    iou=batch_iou,

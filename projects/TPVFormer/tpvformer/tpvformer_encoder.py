...
class TPVFormerOCCEncoder(TransformerLayerSequence):
    """
    ...
    def __init__(self,
        ...
        # embed dims：how many dims to show a signal
        ...
        # LearnedPositionalEncoding(num_feats=128, row_num_embed=100, col_num_embed=100)
        self.positional_encoding = build_positional_encoding(positional_encoding) # 

        tpv_mask_hw = torch.zeros(1, tpv_h, tpv_w)
        self.register_buffer('tpv_mask_hw', tpv_mask_hw)
        # self.real_z 8.0 , self.real_w, self.real_h 102.4
        # num_points_in_pillar [4, 32, 32]
        ...
        self.register_buffer('ref_3d_hw', ref_3d_hw) # torch.Size([1, 4, 10000, 3])
        self.register_buffer('ref_3d_zh', ref_3d_zh) # torch.Size([1, 32, 800, 3])
        self.register_buffer('ref_3d_wz', ref_3d_wz) # torch.Size([1, 32, 800, 3])

        ref_2d_hw = self.get_reference_points(tpv_h, tpv_w, dim='2d', bs=1, device='cpu') # torch.Size([1, 10000, 1, 2])
        ref_2d_zh = self.get_reference_points(tpv_z, tpv_h, dim='2d', bs=1, device='cpu') # torch.Size([1, 800, 1, 2])
        ref_2d_wz = self.get_reference_points(tpv_w, tpv_z, dim='2d', bs=1, device='cpu') # torch.Size([1, 800, 1, 2])
    ...
    def forward(self, mlvl_feats, batch_data_samples):
        ...
        Args:
            mlvl_feats (tuple[Tensor]): Features from the upstream
                ...
                (B, N, C, H, W).
        """
        bs = mlvl_feats[0].shape[0] # 1
        dtype = mlvl_feats[0].dtype # torch.float32
        device = mlvl_feats[0].device # device(type='cuda', index=0)
        ...
        feat_flatten = []
        spatial_shapes = []
        # 1. first flatten the multi-scale features, and use the 
        # cameras embeds features
        for lvl, feat in enumerate(mlvl_feats):
            bs, num_cam, c, h, w = feat.shape
            spatial_shape = (h, w)
            # (bs, num_cam, c, h, w) -> (bs, num_cam, c, h*w) -> (num_cam, bs, h*w, c)
            feat = feat.flatten(3).permute(1, 0, 3, 2)  # num_cam, bs, hw, c torch.Size([6, 1, 23200, 256])
            feat = feat + self.cams_embeds[:, None, None, :].to(dtype )#  [6, 256]
            feat = feat + self.level_embeds[None, None,
                                            lvl:lvl + 1, :].to(dtype) # torch.Size([4, 256]
            spatial_shapes.append(spatial_shape) # spatial_shape (116, 200)
            feat_flatten.append(feat) #  torch.Size([6, 1, 23200, 256])

        # 2. aggregate the camrea multi level features together.
        feat_flatten = torch.cat(feat_flatten, 2)  # num_cam, bs, hw++, c

        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=device)
        # 3. calculate the each level feature start index position.
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1, )), spatial_shapes.prod(1).cumsum(0)[:-1]))
        # 4. dim transpose to (num_cam, H*W, bs, embed_dims)

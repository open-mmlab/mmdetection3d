...
class TPVFormerOCCImageCrossAttention(BaseModule):
    """An attention module used in TPVFormer.

    Args:
        embed_dims (int): Thde embedding dimension of Attention.
            Default: 256.
...
@MODELS.register_module()
class TPVFormerOCCMSDeformableAttention3D(BaseModule):
    ...
    def __init__(
        ...
        # add code different from MSDeformatbleAttention
        # https://github.com/fundamentalvision/BEVFormer/blob/2c889a36c71fa54534032b955ef0463526155951/projects/mmdet3d_plugin/bevformer/modules/spatial_cross_attention.py#L179
    ...
    def forward(self,
        ...
        Args:
            query (Tensor): BEV Query of Transformer, usually from 
                nn.Embedding(),  with shape(bs, num_query, embed_dims).
                num_query is usually equals to the BEV feature 
                width x  BEV feature height.
            key (Tensor): The key tensor with shape
                `(bs, num_key,  embed_dims)`.
            value (Tensor): The value tensor with shape
                `(bs, num_key,  embed_dims)`.
        ...
        if reference_points.shape[-1] == 2:
            ...
            `num_Z_anchors` reference points in each 2D image. 
        ...
        if not self.batch_first: # why use this?

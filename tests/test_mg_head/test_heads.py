...
def test_dcn_center_head():
    ...
    gt_boxes = torch.from_numpy(
        np.load('tests/test_mg_head/gt_boxes.npy')).cuda()
    gt_boxes = gt_boxes[:, [0, 1, 2, 3, 4, 5, 8, 6, 7]]
    ...
    with open('tests/test_mg_head/anno_boxes.pkl', 'rb') as f:
        expected_anno_boxes = pickle.load(f)
    with open('tests/test_mg_head/inds.pkl', 'rb') as f:
    ...
    for i, anno_box in enumerate(anno_boxes):
        assert torch.allclose(
            anno_box,
            torch.from_numpy(expected_anno_boxes[i]).cuda(),
            atol=1e-3)
    for i, ind in enumerate(inds):
        assert torch.all(ind[0,
                             ...] == torch.from_numpy(expected_inds[i]).cuda())
    ...
    loss = dcn_center_head.loss([gt_boxes, gt_boxes], [gt_classes, gt_classes],
                                output)
